...
export const MovieCard = ({ movie, closeModal }: Props) => {
    return (
        <div>
            {movie.map(item =>(   
                <div key={item.id} className="w-screen h-1/3 grid -mt-5 py-6 bg-[#202020]">
                    <div 
                        className="text-white text-xl font-bold mr-8 p-2 cursor-pointer rounded-full border-3 inline place-self-end" 
                        onClick={closeModal}>X
                    </div>
                    {movie.length !== 0 &&
                        <div key={item.id} className="w-3xl flex gap-4 m-auto text-white">
                            <div className="w-64">
